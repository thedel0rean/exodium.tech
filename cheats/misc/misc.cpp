...
#define VERSION crypt_str("govnopasta")
...
void misc::watermark()
{
		...
		std::string name_cheat = crypt_str("FREZZYV4");
#ifdef _DEBUG
		...
		render::get().rect_filled(width - 10 - box_width, 10, box_width, 2, Color(g_cfg.misc.watermark_color));
		render::get().rect_filled(width - 10 - box_width, 11, box_width, 18, Color(0, 0, 0, 25));
...
void misc::fast_stop(CUserCmd* m_pcmd)
{
	...
	if (!((antiaim::get().type == ANTIAIM_LEGIT ? g_cfg.antiaim.desync : g_cfg.antiaim.desync) && (antiaim::get().type == ANTIAIM_LEGIT ? !g_cfg.antiaim.legit_lby_type : !g_cfg.antiaim.lby_type) && (!g_ctx.globals.weapon->is_grenade() || g_cfg.esp.on_click & !(m_pcmd->m_buttons & IN_ATTACK) && !(m_pcmd->m_buttons & IN_ATTACK2))) || antiaim::get().condition(m_pcmd))
	{
...
bool misc::double_tap(CUserCmd* m_pcmd)
{
	...
	{
		auto recharge_time = g_ctx.globals.weapon->can_double_tap() ? TIME_TO_TICKS(0.50f) : TIME_TO_TICKS(1.0f);
	...
	if (!g_ctx.globals.weapon->is_knife() && !g_ctx.globals.weapon->is_grenade() && g_ctx.globals.weapon->m_iItemDefinitionIndex() != WEAPON_TASER && g_ctx.globals.weapon->m_iItemDefinitionIndex() != WEAPON_REVOLVER && (m_pcmd->m_buttons & IN_ATTACK || m_pcmd->m_buttons & IN_ATTACK2 && g_ctx.globals.weapon->is_knife()))
	{
...
void misc::lagcompexploit(CUserCmd* m_pcmd)
{
	...
	if (g_cfg.ragebot.exploit_breaklagcomp && !(m_pcmd->m_buttons & IN_ATTACK)) {
...
void misc::FastFeautres(CUserCmd* pCmd) {

	auto pWeapon = g_ctx.local()->m_hActiveWeapon().Get();

	if (!pWeapon || !g_ctx.local() || !g_ctx.local()->is_alive()) return;


	if (pCmd->m_buttons & IN_ATTACK) {
		if (pWeapon->is_sniper()) {

			if (g_ctx.local()->m_bIsScoped() && g_cfg.misc.misc_fastzoom) {
				pCmd->m_buttons &= ~IN_ATTACK;
				pCmd->m_buttons |= IN_ZOOM;
			}

			if ((pCmd->m_buttons & IN_ATTACK) && g_cfg.misc.misc_fastswitch) {
				m_engine()->ExecuteClientCmd("lastinv");
				m_engine()->ExecuteClientCmd("lastinv");
			}
		}
	}
...
void misc::JumpThrow(CUserCmd* Cmd)
{
	if (!g_ctx.local())
		return;
	...
	if (g_cfg.misc.jumpthrow && pWeapon->is_grenade() && !(Cmd->m_buttons & IN_ATTACK) && !(g_ctx.local()->m_fFlags() & FL_ONGROUND) && g_ctx.local()->m_vecVelocity().z > 265.f)
	{
